...
class Database:
    ...
    def init_database(self):
        ...
        try:
            for website in self.websites:
                existing = (
                    session.query(CollectionStats).filter_by(website=website).first()
                )
                if not existing:
    ...
    def save_trace(self, website, site_idx, trace_data):
        ...
        try:
            session.add(
                Fingerprint(
                    website=website, website_index=site_idx, trace_data=trace_json
                )
            )
    ...
    def export_to_json(self, output_path="dataset.json"):
        ...
        try:
            fingerprints = (
                ...
                .order_by(Fingerprint.website, Fingerprint.id)
                .all()
            )
...
def get_traces_with_labels(db_path="webfingerprint.db"):
    """Load traces and labels from the database for training."""
    ...
    try:
        ...
        for fp in fingerprints:
            try:
                ...
                # Map website to index
                if fp.website not in website_to_index:
                    website_to_index[fp.website] = len(websites)
                    websites.append(fp.website)

                labels.append(website_to_index[fp.website])

            except Exception as e:
                print(f"Error parsing trace data for {fp.website}: {e}")
                continue
